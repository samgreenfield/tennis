...
from .utils import euclidean_distance, bbox_feet
    ...
    def choose_players(self, court_keypoints, player_dict):
        ...
        for track_id, bbox in player_dict.items():
            ...
            for idx, keypoint in enumerate(court_keypoints[0]):
                distance_to_keypoint = euclidean_distance(player_feet, keypoint)
                if distance_to_keypoint < min_distance:
                    min_distance = distance_to_keypoint
            distances.append((track_id, min_distance))

        distances.sort(key=lambda x: x[1], reverse=False)
        chosen_players = [player_id for player_id, min_distance in distances[:2]]
